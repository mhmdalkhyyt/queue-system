...
def _make_zmq_pollitem(socket, flags):
    ...
    zmq_pollitem.backend_socket = zmq_socket
    zmq_pollitem.fd = 0
...
def _make_zmq_pollitem_fromfd(socket_fd, flags):
    zmq_pollitem = ffi.new('zmq_pollitem_t*')
    zmq_pollitem.backend_socket = ffi.NULL
...
def zmq_poll(sockets, timeout):
    ...
    for index in range(len(items)):
        if items[index].revents > 0:
            if not items[index].backend_socket == ffi.NULL:
                result.append(
                    (
                        low_level_to_socket_obj[items[index].backend_socket][0],
                        items[index].revents,
                    )
                )
            else:
