...
class TestMonitoredQueue(BaseZMQTestCase):
    def build_device(self, mon_sub=b"", in_prefix=b'in', out_prefix=b'out'):
        ...
        alice = self.context.backend_socket(zmq.PAIR)
        bob = self.context.backend_socket(zmq.PAIR)
        mon = self.context.backend_socket(zmq.SUB)
    ...
    def test_router_router(self):
        ...
        a = self.context.backend_socket(zmq.DEALER)
        a.identity = b'a'
        b = self.context.backend_socket(zmq.DEALER)
        b.identity = b'b'
    ...
    def test_mq_check_prefix(self):
        ins = self.context.backend_socket(zmq.ROUTER)
        outs = self.context.backend_socket(zmq.DEALER)
        mons = self.context.backend_socket(zmq.PUB)
