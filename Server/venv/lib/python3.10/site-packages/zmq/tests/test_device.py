...
class TestDevice(BaseZMQTestCase):
    ...
    def test_single_socket_forwarder_connect(self):
        ...
        req = self.context.backend_socket(zmq.REQ)
    ...
    def test_proxy(self):
        ...
        time.sleep(0.25)
        msg = b'hello'
        push = self.context.backend_socket(zmq.PUSH)
        push.connect("%s:%i" % (iface, port))
        pull = self.context.backend_socket(zmq.PULL)
        pull.connect("%s:%i" % (iface, port2))
        mon = self.context.backend_socket(zmq.PULL)
...
if have_gevent:
    ...
    class TestDeviceGreen(GreenTest, BaseZMQTestCase):
        def test_green_device(self):
            rep = self.context.backend_socket(zmq.REP)
            req = self.context.backend_socket(zmq.REQ)
