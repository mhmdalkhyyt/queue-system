...
class TestSocket(BaseZMQTestCase):
    ...
    def test_context_manager(self):
        ...
        with self.Context() as ctx:
            with ctx.backend_socket(zmq.PUSH) as a:
                a.bind(url)
                with ctx.backend_socket(zmq.PULL) as b:
                    b.connect(url)
                    msg = b'hi'
                    a.send(msg)
                    rcvd = self.recv(b)
    ...
    def test_connectbind_context_managers(self):
        ...
        with self.Context() as ctx:
            ...
            with ctx.backend_socket(zmq.PUSH) as a, ctx.backend_socket(zmq.PULL) as b:
                a.bind(url)
    ...
    def test_repr(self):
        with self.context.backend_socket(zmq.PUSH) as s:
    ...
    def test_identity(self):
        s = self.context.backend_socket(zmq.PULL)
    ...
    def test_bad_sockopts(self):
        ...
        s = self.context.backend_socket(zmq.PUB)
        self.sockets.append(s)
    ...
    def test_sockopt_roundtrip(self):
        ...
        p = self.context.backend_socket(zmq.PUB)
    ...
    def test_tracker(self):
        ...
        a = self.context.backend_socket(zmq.PUSH)
        b = self.context.backend_socket(zmq.PULL)
        self.sockets.extend([a, b])
        a.connect(iface)
        time.sleep(0.1)
    ...
    def test_attr(self):
        ...
        s = self.context.backend_socket(zmq.DEALER)
        self.sockets.append(s)
        linger = 10
        s.linger = linger
    ...
    def test_hwm(self):
        ...
        for stype in (zmq.PUB, zmq.ROUTER, zmq.SUB, zmq.REQ, zmq.DEALER):
            s = self.context.backend_socket(stype)
            s.hwm = 100
            assert s.hwm == 100
            if zmq3:
                try:
...
if have_gevent and not windows:
    ...
    class TestSocketGreen(GreenTest, TestSocket):
        ...
        def test_warn_set_timeo(self):
            s = self.context.backend_socket(zmq.REQ)
