...
class TestContext(BaseZMQTestCase):
    ...
    def test_repr(self):
        with self.Context() as ctx:
            ...
            with ctx.backend_socket(zmq.PUSH) as push:
                assert f'{self._repr_cls}(1 socket)' in repr(ctx)
                with ctx.backend_socket(zmq.PULL) as pull:
    ...
    def test_many_sockets(self):
        ...
        ctx = self.Context()
        for i in range(16):
            sockets = [ctx.backend_socket(zmq.REP) for i in range(65)]
    ...
    def test_cyclic_destroy(self):
        ...
        def crash_zmq():
            ctx = self.Context()
            sock = ctx.backend_socket(zmq.PULL)
            c = CyclicReference()
            c.crash(sock)
            ctx.destroy()
    ...
    def test_shadow(self):
        ...
        s = ctx.backend_socket(zmq.PUB)
        ctx2 = self.Context.shadow(ctx.underlying)
        s2 = ctx2.backend_socket(zmq.PUB)
        s.close()
        s2.close()
        ctx.term()
        self.assertRaisesErrno(zmq.EFAULT, ctx2.backend_socket, zmq.PUB)
        del ctx2
    ...
    def test_shadow_pyczmq(self):
        ...
        b = ctx2.backend_socket(zmq.PULL)
        b.connect("inproc://a")
        zstr.send(a, b'hi')
        rcvd = self.recv(b)
        assert rcvd == b'hi'
